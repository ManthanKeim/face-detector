...
specs_ori = cv2.imread('123456.png', -1)
cigar_ori = cv2.imread('weed.png', -1)
chain_ori = cv2.imread('chain.png', -1)
...
while 1:
    ...
    for (x, y, w, h) in faces:
        if h > 0 and w > 0:
            ...
            chain_symin = int(y + 4 * h / 6)
            chain_symax = int(y + 5.5 * h / 6)
            ...
            # transparentOverlay(face_chain_roi_color, chain, (int(w / 2), int(sh_chain / 2)))
